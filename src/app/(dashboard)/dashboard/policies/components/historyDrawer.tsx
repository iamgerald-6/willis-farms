...
import { PolicyAuditLogEntry } from "@/types";

const ACTION_LABEL: Record<PolicyAuditLogEntry["action"], string> = {
  added: "Added",
...
export default function PolicyHistoryDrawer({
  manualId,
  manualTitle,
  onClose,
}: {
  ...
  const { data, isLoading, isError, error } = useQuery<{
    entries: PolicyAuditLogEntry[];
  }>({
    queryKey: ["policy-audit", manualId],
    queryFn: async () => (await api.get(`/policies/${manualId}/audit`)).data,
  });
        ...
        <div className="flex items-start justify-between p-5 border-b border-gray-100">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{manualTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
