...
import { SOP_DESCRIPTION_MAX_CHARS } from "@/lib/moduleRegistry";
...
const EXTRACTION_TOOL = {
  name: "record_extracted_sop_details",
  description:
    "Records a suggested title and description for a Wills Farms Standard Operating Procedure (SOP) document, based on what the document actually says.",
  input_schema: {
    ...
    properties: {
      title: {
        type: "string",
        description:
          "A short, clear title for this SOP — use the document's own title/heading if it has one, otherwise summarize what procedure it covers in a few words.",
      },
      description: {
        type: "string",
        description: `A concise, plain-language summary of what this SOP covers and when staff would use it — no more than ${SOP_DESCRIPTION_MAX_CHARS} characters.`,
      },
    },
...
const INSTRUCTIONS =
  "This is a Standard Operating Procedure (SOP) document used internally at Wills Farms, a farm operation in Ghana. Read it and call the record_extracted_sop_details tool with a short, accurate title and a concise description of what the procedure covers and when it's used. Base both only on what the document actually says — don't invent details it doesn't contain.";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      tool_choice: { type: "tool", name: "record_extracted_sop_details" },
    ...
    const description = String(raw.description ?? "")
      .trim()
      .slice(0, SOP_DESCRIPTION_MAX_CHARS);
    ...
    console.error("[POST /api/sop/extract]", err);
