...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const facts = [
      ...
      `- Notice period (for termination clause): ${ctx.noticePeriodDisplay || "[HR TO COMPLETE]"}`,
      ctx.noticePeriod && ctx.noticePeriodFrequency
        ? `- Notice period is ${ctx.noticePeriod} ${ctx.noticePeriodFrequency} — write it exactly as "${ctx.noticePeriodDisplay}" in the Terms of Employment clause; do not substitute days, weeks, or months.`
        : "",
    ...
    const prompt = [
      ...
      "6. A section headed exactly \"Terms of Employment:\" with one paragraph noting the employment is governed by the labour laws of Ghana and may be terminated by either party with written notice of exactly the notice period stated above (number and unit combined — e.g. \"12 Week(s)\" — do not rephrase or pick a different unit).",
    ...
    if (!toolBlock || toolBlock.type !== "tool_use") {
      return NextResponse.json({ error: "WillsOne Intel did not return an offer letter." }, { status: 502 });
    }

    const input = toolBlock.input as { letter_body?: string };
    const letterBody = input.letter_body?.trim();
    if (!letterBody) {
      return NextResponse.json({ error: "WillsOne Intel returned an empty offer letter." }, { status: 502 });
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        context: {
          ...
          notice_period: ctx.noticePeriodDisplay ?? ctx.noticePeriod ?? null,
          notice_period_frequency: ctx.noticePeriodFrequency ?? null,
